...
import { SplineInterpolator } from "../../core/spline";
import type { GLFX } from "../../glfx";
import { clamp } from "../../utils";

function splineInterpolate(points: number[][]) {
  const interpolator = new SplineInterpolator(points);
  const array = [];
  for (let i = 0; i < 256; i++) {
    array.push(
      clamp(0, Math.floor(interpolator.interpolate(i / 255) * 256), 255)
    );
  }
  return array;
}
...
export const curves = (
  ...
  red = splineInterpolate(red as number[][]);
  if (!green || !blue) {
    green = blue = red;
  } else {
    green = splineInterpolate(green as number[][]);
    blue = splineInterpolate(blue as number[][]);
  }
  ...
  glfx.extraTexture?.initFromBytes(256, 1, array);
  ...
  glfx.curvesS.textures({
    map: 1,
  });

  glfx.simpleShader.call(this, glfx.curvesS, {});
